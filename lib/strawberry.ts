...
export function drawStrawberry(
  ...
  gradient.addColorStop(0, "#ff806f");
  gradient.addColorStop(0.45, "#f4513d");
  gradient.addColorStop(1, "#d92f25");

  ctx.fillStyle = gradient;
  ctx.fill();

  ctx.lineWidth = Math.max(1, size * 0.025);
  ctx.strokeStyle = "#b92820";
  ctx.stroke();
  ...
  ctx.fillStyle = "#4caf63";
  ctx.strokeStyle = "#2e7541";
  ctx.lineWidth = Math.max(1, size * 0.018);

  const leafAngles = [-65, -32, 0, 32, 65];
...
/*
 * Pre-render strawberry ke canvas.
 * Berguna kalau nanti ingin dipakai sebagai sprite.
 */
export function loadStrawberrySprite(
  size = 160
): HTMLCanvasElement {
  const canvas = document.createElement("canvas");
